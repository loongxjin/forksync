...
func TestManager_ResolveConflicts(t *testing.T) {
	...
	result, err := mgr.ResolveConflicts(context.Background(), "repo-1", "/path/to/repo", []string{"a.go", "b.go"}, "preserve_ours", nil)
	if err != nil {
...
func TestManager_ResolveConflicts_NoSession(t *testing.T) {
	...
	result, err := mgr.ResolveConflicts(context.Background(), "repo-new", "/path/to/repo", []string{"a.go"}, "preserve_ours", nil)
	if err != nil {
		t.Fatalf("ResolveConflicts: %v", err)
	}
	if !result.Success {
