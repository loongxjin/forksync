...
import (
	...
	"github.com/loongxjin/forksync/engine/core/notify"
...
func TestTryAgentResolve_NilSessionManager(t *testing.T) {
	...
	out, _, _ := s.tryAgentResolve(context.Background(), repo, []string{"a.go"}, "")
	if out != nil {
		t.Error("out = non-nil, want nil (no session manager)")
	}
}
